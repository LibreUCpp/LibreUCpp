...
struct __attribute__((aligned(4), packed)) USB_OTG_PWRCLK_T
{
    ...
    volatile union FS_PCGCCTL_T // FS_PCGCCTL: OTG_FS power and clock gating control register (OTG_FS_PCGCCTL) 
    {
        uint32_t reg;
...
}; // struct USB_OTG_PWRCLK_T

static_assert(sizeof(USB_OTG_PWRCLK_T) == USB_OTG_PWRCLK_T::PADDED_INSTANCE_SIZE, "invalid peripheral register struct");
