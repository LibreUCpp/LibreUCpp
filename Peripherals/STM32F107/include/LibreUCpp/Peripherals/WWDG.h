...
struct __attribute__((aligned(4), packed)) WWDG_T
{
    ...
    volatile union CR_T // CR: Control register (WWDG_CR) 
    {
        uint32_t reg;
    ...
    volatile union CFR_T // CFR: Configuration register (WWDG_CFR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) CFR_BIT_T
        {
            unsigned W     : 7; // bits 0..6 7-bit window value
            unsigned WDGTB : 2; // bits 7..8 Timer Base
        ...
        static constexpr unsigned WDGTB_POS = 7;
        static constexpr unsigned WDGTB_MASK = 0x00000180;
        static constexpr unsigned WDGTB(unsigned value) { return (value << 7); }
    ...
    volatile union SR_T // SR: Status register (WWDG_SR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) SR_BIT_T
        {
            unsigned EWI : 1; // bit 0 Early Wakeup Interrupt
            unsigned     : 31; // bits 1..31 unused    
        } bit;
    
        static constexpr unsigned EWI_POS = 0;
        static constexpr unsigned EWI_MASK = 0x00000001;
        static constexpr unsigned EWI(unsigned value) { return (value << 0); }
    } SR;

}; // struct WWDG_T

static_assert(sizeof(WWDG_T) == WWDG_T::PADDED_INSTANCE_SIZE, "invalid peripheral register struct");
