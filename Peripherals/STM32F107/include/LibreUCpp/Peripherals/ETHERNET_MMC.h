...
struct __attribute__((aligned(4), packed)) ETHERNET_MMC_T
{
    ...
    volatile union MMCCR_T // MMCCR: Ethernet MMC control register (ETH_MMCCR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) MMCCR_BIT_T
        {
            unsigned CR  : 1; // bit 0 Counter reset
            unsigned CSR : 1; // bit 1 Counter stop rollover
            unsigned ROR : 1; // bit 2 Reset on read
            unsigned     : 28; // bits 3..30 unused
            unsigned MCF : 1; // bit 31 MMC counter freeze    
        } bit;
        ...
        static constexpr unsigned MCF_POS = 31;
        static constexpr unsigned MCF_MASK = 0x80000000;
        static constexpr unsigned MCF(unsigned value) { return (value << 31); }
    } MMCCR;

    volatile union MMCRIR_T // MMCRIR: Ethernet MMC receive interrupt register (ETH_MMCRIR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) MMCRIR_BIT_T
        {
            unsigned       : 5; // bits 0..4 unused
            unsigned RFCES : 1; // bit 5 Received frames CRC error status
            unsigned RFAES : 1; // bit 6 Received frames alignment error status
            unsigned       : 10; // bits 7..16 unused
            unsigned RGUFS : 1; // bit 17 Received Good Unicast Frames Status
    ...
    volatile union MMCTIR_T // MMCTIR: Ethernet MMC transmit interrupt register (ETH_MMCTIR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) MMCTIR_BIT_T
        {
            unsigned         : 14; // bits 0..13 unused
            unsigned TGFSCS  : 1; // bit 14 Transmitted good frames single collision status
            unsigned TGFMSCS : 1; // bit 15 Transmitted good frames more single collision status
            unsigned         : 5; // bits 16..20 unused
            unsigned TGFS    : 1; // bit 21 Transmitted good frames status
    ...
    volatile union MMCRIMR_T // MMCRIMR: Ethernet MMC receive interrupt mask register (ETH_MMCRIMR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) MMCRIMR_BIT_T
        {
            unsigned       : 5; // bits 0..4 unused
            unsigned RFCEM : 1; // bit 5 Received frame CRC error mask
            unsigned RFAEM : 1; // bit 6 Received frames alignment error mask
            unsigned       : 10; // bits 7..16 unused
            unsigned RGUFM : 1; // bit 17 Received good unicast frames mask
            unsigned       : 14; // bits 18..31 unused    
        } bit;
    ...
    volatile union MMCTIMR_T // MMCTIMR: Ethernet MMC transmit interrupt mask register (ETH_MMCTIMR) 
    {
        uint32_t reg;
        struct __attribute__((packed)) MMCTIMR_BIT_T
        {
            unsigned         : 14; // bits 0..13 unused
            unsigned TGFSCM  : 1; // bit 14 Transmitted good frames single collision mask
            unsigned TGFMSCM : 1; // bit 15 Transmitted good frames more single collision mask
            unsigned         : 5; // bits 16..20 unused
            unsigned TGFM    : 1; // bit 21 Transmitted good frames mask
            unsigned         : 10; // bits 22..31 unused    
        } bit;
        ...
        static constexpr unsigned TGFM_POS = 21;
        static constexpr unsigned TGFM_MASK = 0x00200000;
        static constexpr unsigned TGFM(unsigned value) { return (value << 21); }
    } MMCTIMR;
    ...
    volatile union MMCTGFSCCR_T // MMCTGFSCCR: Ethernet MMC transmitted good frames after a single collision counter 
    {
        ...
        {
            unsigned TGFSCC : 32; // bits 0..31 Transmitted good frames after a single collision counter    
        } bit;
    ...
    volatile union MMCTGFMSCCR_T // MMCTGFMSCCR: Ethernet MMC transmitted good frames after more than a single collision 
    {
        ...
        {
            unsigned TGFMSCC : 32; // bits 0..31 Transmitted good frames after more than a single collision counter    
        } bit;
    ...
    volatile union MMCTGFCR_T // MMCTGFCR: Ethernet MMC transmitted good frames counter register 
    {
        ...
        {
            unsigned TGFC : 32; // bits 0..31 Transmitted good frames counter    
        } bit;
    ...
    volatile union MMCRFCECR_T // MMCRFCECR: Ethernet MMC received frames with CRC error counter register 
    {
        ...
        {
            unsigned RFCFC : 32; // bits 0..31 Received frames with CRC error counter    
        } bit;
    ...
    volatile union MMCRFAECR_T // MMCRFAECR: Ethernet MMC received frames with alignment error counter register 
    {
        ...
        {
            unsigned RFAEC : 32; // bits 0..31 Received frames with alignment error counter    
        } bit;
    ...
    volatile union MMCRGUFCR_T // MMCRGUFCR: MMC received good unicast frames counter register 
    {
        ...
        {
            unsigned RGUFC : 32; // bits 0..31 Received good unicast frames counter    
        } bit;
