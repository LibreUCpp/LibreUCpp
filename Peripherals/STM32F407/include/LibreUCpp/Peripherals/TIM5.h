...
struct __attribute__((aligned(4), packed)) TIM5_T
{
    ...
    static constexpr intptr_t ADDR_OFFSET_OR = 0x50;
    ...
    volatile union CCMR1_Output_T // CCMR1_Output: capture/compare mode register 1 (output mode) 
    {
        ...
        {
            unsigned CC1S  : 2; // bits 0..1 CC1S
            unsigned OC1FE : 1; // bit 2 OC1FE
            unsigned OC1PE : 1; // bit 3 OC1PE
            unsigned OC1M  : 3; // bits 4..6 OC1M
            unsigned OC1CE : 1; // bit 7 OC1CE
            unsigned CC2S  : 2; // bits 8..9 CC2S
            unsigned OC2FE : 1; // bit 10 OC2FE
            unsigned OC2PE : 1; // bit 11 OC2PE
            unsigned OC2M  : 3; // bits 12..14 OC2M
            unsigned OC2CE : 1; // bit 15 OC2CE
            unsigned       : 16; // bits 16..31 unused    
        } bit;
    ...
    volatile union CCMR2_Output_T // CCMR2_Output: capture/compare mode register 2 (output mode) 
    {
        ...
        {
            unsigned CC3S  : 2; // bits 0..1 CC3S
            unsigned OC3FE : 1; // bit 2 OC3FE
            unsigned OC3PE : 1; // bit 3 OC3PE
            unsigned OC3M  : 3; // bits 4..6 OC3M
            unsigned OC3CE : 1; // bit 7 OC3CE
            unsigned CC4S  : 2; // bits 8..9 CC4S
            unsigned OC4FE : 1; // bit 10 OC4FE
            unsigned OC4PE : 1; // bit 11 OC4PE
            unsigned OC4M  : 3; // bits 12..14 OC4M
            unsigned O24CE : 1; // bit 15 O24CE
            unsigned       : 16; // bits 16..31 unused    
        } bit;
        ...
        static constexpr unsigned O24CE_POS = 15;
        static constexpr unsigned O24CE_MASK = 0x00008000;
        static constexpr unsigned O24CE(unsigned value) { return (value << 15); }
    } CCMR2_Output;
    ...
    volatile union CNT_T // CNT: counter 
    {
        ...
        struct __attribute__((packed)) CNT_BIT_T
        {
            ...
            unsigned CNT_H : 16; // bits 16..31 High counter value    
        } bit;
    ...
    volatile union ARR_T // ARR: auto-reload register 
    {
        ...
        struct __attribute__((packed)) ARR_BIT_T
        {
            ...
            unsigned ARR_H : 16; // bits 16..31 High Auto-reload value    
        } bit;
    ...
    volatile union CCR1_T // CCR1: capture/compare register 1 
    {
        ...
        struct __attribute__((packed)) CCR1_BIT_T
        {
            ...
            unsigned CCR1_H : 16; // bits 16..31 High Capture/Compare 1 value    
        } bit;
    ...
    volatile union CCR2_T // CCR2: capture/compare register 2 
    {
        ...
        struct __attribute__((packed)) CCR2_BIT_T
        {
            ...
            unsigned CCR2_H : 16; // bits 16..31 High Capture/Compare 2 value    
        } bit;
    ...
    volatile union CCR3_T // CCR3: capture/compare register 3 
    {
        ...
        struct __attribute__((packed)) CCR3_BIT_T
        {
            ...
            unsigned CCR3_H : 16; // bits 16..31 High Capture/Compare value    
        } bit;
    ...
    volatile union CCR4_T // CCR4: capture/compare register 4 
    {
        ...
        struct __attribute__((packed)) CCR4_BIT_T
        {
            ...
            unsigned CCR4_H : 16; // bits 16..31 High Capture/Compare value    
        } bit;
    ...
    volatile union OR_T // OR: TIM5 option register 
    {
        uint32_t reg;
        struct __attribute__((packed)) OR_BIT_T
        {
            unsigned         : 6; // bits 0..5 unused
            unsigned IT4_RMP : 2; // bits 6..7 Timer Input 4 remap
            unsigned         : 24; // bits 8..31 unused    
        } bit;
    
        static constexpr unsigned IT4_RMP_POS = 6;
        static constexpr unsigned IT4_RMP_MASK = 0x000000c0;
        static constexpr unsigned IT4_RMP(unsigned value) { return (value << 6); }
    } OR;

}; // struct TIM5_T

static_assert(sizeof(TIM5_T) == TIM5_T::PADDED_INSTANCE_SIZE, "invalid peripheral register struct");
