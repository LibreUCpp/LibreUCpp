...
struct __attribute__((aligned(4), packed)) TIM2_T
{
    ...
    volatile union OR_T // OR: TIM5 option register 
    {
        ...
        {
            unsigned          : 10; // bits 0..9 unused
            unsigned ITR1_RMP : 2; // bits 10..11 Timer Input 4 remap
            unsigned          : 20; // bits 12..31 unused    
        } bit;
    
        static constexpr unsigned ITR1_RMP_POS = 10;
        static constexpr unsigned ITR1_RMP_MASK = 0x00000c00;
        static constexpr unsigned ITR1_RMP(unsigned value) { return (value << 10); }
    } OR;

}; // struct TIM2_T

static_assert(sizeof(TIM2_T) == TIM2_T::INSTANCE_SIZE, "invalid peripheral register struct");
