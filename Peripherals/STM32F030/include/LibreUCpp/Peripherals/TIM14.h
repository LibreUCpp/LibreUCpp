...
struct __attribute__((aligned(4), packed)) TIM14_T
{
    ...
    volatile union OR_T // OR: option register 
    {
        uint32_t reg;
        struct __attribute__((packed)) OR_BIT_T
        {
            unsigned RMP : 2; // bits 0..1 Timer input 1 remap
            unsigned     : 30; // bits 2..31 unused    
        } bit;
    
        static constexpr unsigned RMP_POS = 0;
        static constexpr unsigned RMP_MASK = 0x00000003;
        static constexpr unsigned RMP(unsigned value) { return (value << 0); }
    } OR;

}; // struct TIM14_T

static_assert(sizeof(TIM14_T) == TIM14_T::PADDED_INSTANCE_SIZE, "invalid peripheral register struct");
